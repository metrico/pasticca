...
func (s *Server) handlePostRequest(c *gin.Context) {
        ...
        // Update database
        _, err = s.db.Exec("INSERT INTO paste_references (path, fingerprint, hash) VALUES ($1, $2, $3) ON CONFLICT (path) DO UPDATE SET fingerprint = $2, hash = $3", requestPath, fingerprint, hash)
        if err != nil {
